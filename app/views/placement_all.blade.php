                     ...
                     <h3>Training & placement</h3>
                         <div class="col-md-3 notice_all">
                             <ul class="list-unstyled">
                                 ...
                                 <li><a href="{{URL::to('placement_month/'.$data->id)}}">{{date('M-Y',strtotime($data->event_date))}} {{'('.$data->total.')'}}</a></li>
                                 @endforeach
                             </ul>
                         </div>
                            ...
                            <ul >
                                 @foreach($content['notice_by_date'] as $data)
                                  <li>
                                      ...
                                      <i>Posted at</i> :- <strong>{{date('d-M-Y',strtotime($data->event_date))}}</strong>

                                      <p>
                                          {{substr(strip_tags($data->description),0,100)}}......
                                      </p>
                                      <p><a href="{{URL::to('placement/'.$data->id)}}">Read more </a></p>
                                  </li>
                                 @endforeach
                            </ul>
                         </div>
